...
export function DataTableCvApplications({ data: init }: { data: any[] }) {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Οριστική διαγραφή προφίλ υποψηφίου;")) return;
        try {
            await deleteCvApplication(id);
            setData((prev: any[]) => prev.filter(d => d.id !== id));
            toast.success("Το αρχείο αιτήσεως διαγράφηκε");
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-[#605E5C]">
                        {row.original.position?.titleEL || <span className="text-[#C8C6C4] italic">Γενική Αίτηση</span>}
                    </span>
                </div>
            )
        },
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem className="text-sm" onClick={() => window.open(row.original.cvUrl, "_blank")}>
                            <FileSpreadsheet className="w-3.5 h-3.5 mr-2 text-[#0078D4]" /> Προβολή Βιογραφικού
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <div className="px-2 py-1 text-[10px] font-bold uppercase text-[#A19F9D] tracking-widest">Ενημέρωση Κατάστασης</div>
                        {["REVIEWED", "SHORTLISTED", "REJECTED"].map(s => (
                            <DropdownMenuItem key={s} className="text-sm" onClick={async () => {
                                ...
                                Σήμανση ως {s === "REVIEWED" ? "Αξιολογήθηκε" : s === "SHORTLISTED" ? "Επιλέχθηκε" : "Απορρίφθηκε"}
                            </DropdownMenuItem>
                        ))}
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleDelete(row.original.id)} className="text-sm text-red-500 focus:bg-red-50 focus:text-red-600">
                            <UserX className="w-3.5 h-3.5 mr-2" /> Αφαίρεση Αίτησης
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
            )
    ...
    return (
        <div className="space-y-4">
            <GenericDataTable
                columns={columns}
                data={data}
                searchPlaceholder="Αναζήτηση υποψηφίου με όνομα..."
                    ...
                    <div className="mx-4 mb-3 mt-1 rounded-lg border border-[#EDEBE9] bg-[#F3F2F1] overflow-hidden">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[#EDEBE9]">
                            <div className="bg-white p-4 space-y-3">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[#A19F9D] mb-3">Στοιχεία Υποψηφίου</p>
                                ...
                                <div className="pt-3 border-t border-[#EDEBE9] space-y-1.5">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-[#605E5C]">
                                        ...
                                        Υπέβαλε: {new Date(item.createdAt).toLocaleDateString()}
                                    </div>
                                </div>
                            </div>
                            <div className="bg-white p-4 space-y-3">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-[#A19F9D] mb-3">Συνοδευτική Επιστολή</p>
                                <p className="text-sm leading-relaxed text-[#605E5C] italic">
                                    {item.coverLetter ? `"${item.coverLetter}"` : <span className="not-italic text-[#A19F9D]">Δεν υποβλήθηκε συνοδευτική επιστολή.</span>}
                                </p>
                                <div className="pt-3 border-t border-[#EDEBE9]">
                                    ...
                                    >
                                        <FileSpreadsheet className="w-3.5 h-3.5 text-[#0078D4]" /> Λήψη Βιογραφικού
                                    </Button>
                                </div>
                            </div>
                        </div>
